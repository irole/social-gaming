...
class ShopCategoryValidator extends Validator {
        ...
        return [
            body('name').not().isEmpty().trim().escape().withMessage((value, {req}) => {
                ...
                if (req.query._method === 'put') {
                    let category = await shopCategoryService.findById(req.params.id);
                    if (category.name === value) return;
                }

                let category = await shopCategoryService.findOne({name: value});
                if (category) {
...
export default new ShopCategoryValidator();
